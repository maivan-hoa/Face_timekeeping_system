...
from FACE_RECOGNITION.sort import Sort
import numpy as np
import time
import random
import collections
from FACE_RECOGNITION.TDDFA_OPENVINO import TDDFA_OPENVINO
from FACE_RECOGNITION.utils.box_utils import parse_roi_box_from_bbox, get_poses
from FACE_RECOGNITION.model_detection_test import FaceDetector
from FACE_RECOGNITION.vectorization import Vectorization
from FACE_RECOGNITION.matching import Matching
import datetime
import json
import os
import sys
...
def run(flag,type_cam,path_detection,path_vectori,path_to_headpose,folder_vector, time_open=60):
    print(cwd)
    print(cwd1)
    # global time_open
    start = time.time()
    ...
    if not os.path.exists(cwd1+ "/image_to_debug/"+day_now):
        os.makedirs(cwd1+ "/image_to_debug/"+day_now)
    if not os.path.exists(cwd+ "/results/"+day_now):
        os.makedirs(cwd+ "/results/"+day_now)
    tracker = Sort()
    ...
    matching_model= Matching(type_cam,0.4,0.45,0.6,gallery,gallery_id)

    cap=cv2.VideoCapture(cwd+"/video_test.mp4")
    ...
    while(True):
            ...
            if(len(dead_track)>0):
                ...
                for id_dead in dead_track:                 
                    ...
                    if(id_name==-1):       
                        ...
                        cv2.imwrite(cwd1+ '/image_to_debug/'+day_now+"/"+str(time_checkin)+"_"+str(count_nguoila)+"_"+str(flag)+".jpg",img[ymin:ymax,xmin:xmax]) 
                    # if(flag==0):
        ...
        #     break
        if cv2.waitKey(1) & (time.time() - start > time_open):
            break

    if(flag==0):
        for i in nhanvien_ok:
            nhanvien_ok[i]=nhanvien_ok[i].split(',')[0]
        with open(cwd+ '/results/'+day_now+"/"+'morning.json', 'w') as fp:
            json.dump(nhanvien_ok, fp)
    if(flag==1):
        for i in nhanvien_ok:
            nhanvien_ok[i]=nhanvien_ok[i].split(',')[-1]
        with open(cwd+ '/results/'+day_now+"/"+'afternoon.json', 'w') as fp:
            json.dump(nhanvien_ok, fp)
...
if __name__ == '__main__':
    ...
    start_time_in = '16:25:00'
    end_time_in = '16:26:00'

    # time_open = (datetime.datetime.strptime(end_time_in, '%H:%M:%S') - datetime.datetime.strptime(start_time_in, '%H:%M:%S')).total_seconds()
    time_open = 60

    schedule.every().day.at(start_time_in[:-3]).do(start)
    schedule.every().day.at(end_time_in[:-3]).do(end)
